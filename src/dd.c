...
/* Written by Paul Rubin, David MacKenzie, and Stuart Kemp.
   _WIN32 compatibility layer by Michael Stumpfl. */
...
#else
#define static                    /*windd { // make local variables accessible everywhere */
#endif
...
#if defined _WIN32
#undef static                     /*windd } // make local variables accessible everywhere */
#endif /* defined _WIN32 */
...
"),
    ...
    "Ctrl-Shift-PrintScreen");
      ...
      #if defined _WIN32
      fputs ("\nWindows only:\n", stdout);
      fputs (LIST_OPTION_DESCRIPTION, stdout);
      fputs (NOLCK_OPTION_DESCRIPTION, stdout);
      fputs (NOOPT_OPTION_DESCRIPTION, stdout);
      fputs (FORCE_OPTION_DESCRIPTION, stdout);
      fputs (CONFIRM_OPTION_DESCRIPTION, stdout);
      #endif /* defined _WIN32 */
...
process_signals (void)
{
  ...
  curr_key_up_state = (GetKeyState (VK_SNAPSHOT) < 0);

  if (curr_key_up_state && not prev_key_up_state)
  {
    if (GetKeyState (VK_LCONTROL) && GetKeyState (VK_LSHIFT))
      ++info_signal_count;
  }

  prev_key_up_state = curr_key_up_state;
  #endif /* defined _WIN32 */
  ...
  while (interrupt_signal || info_signal_count)
    {
      ...
      (void)oldset;  /*windd: unreferenced local variable*/
...
main (int argc, char **argv)
{
  ...
  #else
  install_signal_handlers_win32 ();  /*windd*/
  #endif /* defined _WIN32 */
