...
/* Thread-local storage: InitializeWMI() may be called for each thread: */
extern __declspec(thread) bool
  initialized_wmi = false;
...
{ HRESULT
  ...
  if (initialized_wmi || not InitializeCOM ())
    return (initialized_wmi);
  ...
  */
  ...
  { BSTR
    ...
    */
    ...
    hres = CoSetProxyBlanket ( /* WinNT4, Windows 98, Windows 95 with DCOM */
...
{
  return (initialized_wmi && COMAvailable());
}
...
static inline void Free_WbemLocator (void)
{
  CoReleaseObject (acquired_locator);
}


static inline void Free_WbemServices (void)
{
